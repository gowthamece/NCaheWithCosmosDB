...
namespace CosmosGettingStartedTutorial
{
    class Program
    {
        ...
        static ICache _NCacheClient = CacheManager.GetCache("myLocalCache");
        ...
        public async Task GetStartedDemoAsync()
        {
            ...
            family.Id = "Andersen.1";
            await this.GetFamily(family);
        ...
        {
            string key = "SELECT * FROM c WHERE c.PartitionKey = 'Andersen'";
        ...
        private async Task GetFamily(Family family)
        {
            try
            {
                ...
                var familyResult = _NCacheClient.Get<string>(key);

                if (familyResult != null)
                {
                    var result = JsonConvert.DeserializeObject<Family>(familyResult);
                    Console.WriteLine(result.PartitionKey);
                }
                else
                {
                    //var sqlQueryText = $"SELECT * FROM c WHERE C.id={family.Id}";
                    var sqlQueryText = "SELECT * FROM c";
                    QueryDefinition queryDefinition = new QueryDefinition(sqlQueryText);
                    FeedIterator<Family> queryResultSetIterator = this.container.GetItemQueryIterator<Family>(queryDefinition);
                    while (queryResultSetIterator.HasMoreResults)
